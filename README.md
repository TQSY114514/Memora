...
> **本项目仍处于 Beta 阶段。** 虽然版本号已到 1.7.x，但这是快速迭代的产物（一周内从 0.1.0 到 1.7.1），不代表生产级稳定性。
>
> 已完成的安全加固：CSP / 渲染进程沙箱 / safeStorage API Key 加密 / IPC 入参校验 / 导入敏感信息脱敏。
> 待完善：全面 zod schema 校验 / E2E 测试覆盖 / 性能压测。
>
> **建议**：在数据安全要求高的场景，先用少量非敏感对话试跑，确认符合预期后再导入完整历史。
