...
export function upsertSummary(
  sessionId: string,
  data: { summary: string; keyPoints: string[]; todos: string[]; model?: string }
): SessionSummary {
  const db = getDatabase()
  ...
  if (existing) {
    db.prepare(
      `UPDATE session_summaries
       SET summary = ?, key_points = ?, todos = ?, model = ?, updated_at = ?
       WHERE session_id = ?`
    ).run(
      data.summary,
  ...
  db.prepare(
    `INSERT INTO session_summaries
     (id, session_id, summary, key_points, todos, model, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
    sessionId,
